...
import { CleanerService } from "../../../../services/v1/cleaner/createCleaners/createCleanerService";
...
export class CreateCleaners extends OpenAPIRoute {
                    ...
                    "application/json": {
                        schema: z.object({
                            cleaners: z.array(z.object({
                                name: z.string().min(2),
                                zones: z.string().describe("Zonas separadas por virgula"),
                                shift_start: z.string().regex(/^\d{2}:\d{2}$/, "HH:MM"),
                                shift_end: z.string().regex(/^\d{2}:\d{2}$/, "HH:MM"),
                                fixed_accommodations: z.string().optional().describe("Codigos/Nomes dos imoveis fixos"),
                                is_fixed: booleanLike.default(false).describe("Se true, limpa APENAS os imoveis fixos"),
    ...
    async handle(c: Context<{ Bindings: Env }>) {
        ...
        try {
            await service.registerCleaners(cleaners);
